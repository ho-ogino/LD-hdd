...
; cf06にはJP 0xd206(元々0x0006に書かれていたアドレス(LSX-Dodgersのシステム領域開始アドレス)) を書く事で強引に常駐させています。
...
;----------------------------------------------------------------------------
;
LDSYS		equ	0xd006			; 常駐アドレスの頭
LDDEF		equ	0xd206			; 既定のLDのアドレス
...
;----------------------------------------------------------------------------
start:
	; 常駐チェック(手抜き。0x0006に0xd006が書かれていれば常駐済とする……)
	ld	hl,(0x0006)
	ld	a,l
	cp	LDSYS & 0xff
	jr	nz,registhddd
...
registhddd:
	; 常駐チェック2(0x0006に0xd206が書かれていれば常駐する)
	ld	a,l
	cp	LDDEF & 0xff
	jp	nz,0
	ld	a,h
	...
	; SASI読み書き処理をTPA末尾付近(0xd006)にコピーする
...
hdrgmsg:
	db	'LD HDD controller v0.09', 0x0d,0x0a, '$'
targetdrv:
	db	'H: $'
...
; このアドレスが0x0006に書かれている。本来のシステムコールアドレスである0xd206をここで呼ぶ(いいのかこれ)
